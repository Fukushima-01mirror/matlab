% 常にsinの速度によるグループ分け(操作誤差小さい順に60秒間データ)
...
% データグループ分け
    for i = 1: length(excel.num)
        ...
        %dataGroupソート条件
        if ~isempty(strfind( char( excel.txt(i+1,1) ) , '常にsin')) ...         %sin運動　低速
        ...
        %dataGroupソート条件
        if ~isempty(strfind( char( excel.txt(i+1,1) ) , '常にsin')) ...         %sin運動　中速
                && ~isempty(strfind( char( excel.txt(i+1,1) ) , '2013')) ...
                && excel.num(i,3)==0 ...
        ...
        %dataGroupソート条件
        if ~isempty(strfind( char( excel.txt(i+1,1) ) , '常にsin')) ...         %sin運動　高速
                && ~isempty(strfind( char( excel.txt(i+1,1) ) , '2013')) ...
                && excel.num(i,3)==0 ...
