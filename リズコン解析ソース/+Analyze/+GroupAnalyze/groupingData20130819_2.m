% ランダムsinの速度によるグループ分け(操作誤差小さい順に60秒間データ)
function[dataGroup] = groupingData20130819_2(dataGroup,excel)
%input  obj ←　dataGroup
...
% データグループ分け
    for i = 1: length(excel.num)
        ...
        %dataGroupソート条件
        if ~isempty(strfind( char( excel.txt(i+1,1) ) , 'sinランダム')) ...         %ランダムsin運動　低速
                && excel.num(i,3)==0 ...
        ...
        %dataGroupソート条件
        if ~isempty(strfind( char( excel.txt(i+1,1) ) , 'sinランダム')) ...         %ランダムsin運動　中速
                && excel.num(i,3)==0 ...
        ...
        %dataGroupソート条件
        if ~isempty(strfind( char( excel.txt(i+1,1) ) , 'sinランダム')) ...         %ランダムsin運動　高速
                && excel.num(i,3)==0 ...
