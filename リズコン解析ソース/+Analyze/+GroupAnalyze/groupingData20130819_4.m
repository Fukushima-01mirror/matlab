% 定常の速度(sin，等速0.16，等速0.20)によるグループ分け(操作誤差小さい順に60秒間データ)
function[dataGroup] = groupingData20130819_4(dataGroup,excel)
%input  obj ←　dataGroup
...
% データグループ分け
    for i = 1: length(excel.num)
        ...
        %dataGroupソート条件
        if ~isempty(strfind( char( excel.txt(i+1,1) ) , '常にsin'))...
                && excel.num(i,2)-excel.num(i,1)==2500  && excel.num(i,3) == 0
%                 && ~isempty(strfind( char( excel.txt(i+1,1) ) , '0809')) ...         %定常　sin
            suffix = 1;
            dataGroup(suffix).list(j1,:) = excel.raw(i+1,:);
            eval(['j' num2str(suffix) '=j' num2str(suffix) '+1;']);
        end    
        %dataGroupソート条件
        if ~isempty(strfind( char( excel.txt(i+1,1) ) , '等速')) ...         %定常　等速0.16
        ...
        %dataGroupソート条件
        if ~isempty(strfind( char( excel.txt(i+1,1) ) , '等速')) ...         %定常　等速0.20
