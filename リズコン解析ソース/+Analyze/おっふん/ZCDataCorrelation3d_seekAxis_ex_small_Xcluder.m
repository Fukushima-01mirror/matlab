classdef ZCDataCorrelation3d_seekAxis_ex_small_Xcluder < Analyze.Base
    ...
    methods
        function obj = ZCDataCorrelation3d_seekAxis_ex_small_Xcluder(config,data)
        ...
        function runForAlone(obj,user)
           ...
           [zeroCrossTimes] = Rhythm.setZeroCrossCount(user.zeroCrossData);
            ...
            [dA_max,dA_imax] = max(dA_zc);     dT_zc(dA_imax)= [];	 dA_zc(dA_imax)= [];     Y_zc(dA_imax)= [];     Time_zc(dA_imax)= [];
            dA_iout = find (dT_zc > 100 );     dT_zc(dA_iout)= [];	 dA_zc(dA_iout)= [];     Y_zc(dA_iout)= [];     Time_zc(dA_iout)= [];
            dT_iout = find (dA_zc > 100 );     dT_zc(dT_iout)= [];	 dA_zc(dT_iout)= [];     Y_zc(dT_iout)= [];     Time_zc(dT_iout)= [];
%             [Y_max,Y_imax] = max(Y_zc);     dT_zc(Y_imax)= [];	 dA_zc(Y_imax)= [];     Y_zc(Y_imax)= [];
%             [Y_max,Y_imax] = max(Y_zc);     dT_zc(Y_imax)= [];	 dA_zc(Y_imax)= [];     Y_zc(Y_imax)= [];
%             while max(Y_zc./dT_zc) > 500
%                 [Y_dT_max,Y_dT_imax] = max(Y_zc./dT_zc);     dT_zc(Y_udT_imax)= [];	 dA_zc(Y_dT_imax)= [];     Y_zc(Y_dT_imax)= [];
