classdef ZCDataAvtPosVar3 < Analyze.Base
    ...
    methods
        function obj = ZCDataAvtPosVar3(config,data)
...
% 
%             fid = fopen('dAdata.csv','A');
%             fprintf(fid,'%d',dA_zc);
%             fprintf(fid,'%d',dT_zc);
%             fprintf(fid,'%d',Time_zc);
%             fclose(fid);
%             
            csvwrite('dAdata.csv',dA_zc);
            csvwrite('Timedata.csv',Time_zc);
            csvwrite('dTdata.csv',dT_zc);
...
%%        回帰直線からの距離の時間変化
           ...
           %%剣角度検出
            ...
            for j= 2:length(SA)
             ...
             xlabel('時間t ms'); ylabel('剣角度');
              ...
              a2 = subplot( 3,1,2);
              plot(   Time_zc, dA_zc ,'-r.','MarkerEdgeColor','k','MarkerSize',3);
              hold on
              for j= 2:length(SA)
                SupTime = user.time.lowSampled(j);
                if SA(j-1) == 0 && SA(j) ~= SA(j-1)
 ...
 %                   disp(SupTime);
            ...
            hold off
             a3 = subplot( 3,1,3);
              plot(   Time_zc, dT_zc,'-r.','MarkerEdgeColor','k','MarkerSize',3);
              hold on
               for j= 2:length(SA)
                SupTime = user.time.lowSampled(j);
                if SA(j-1) == 0 && SA(j) ~= SA(j-1)
