classdef ZCDataCorrelation3d_approxiFlatSurf_ex2_HalfHalf < Analyze.Base
    ...
    methods
        function obj = ZCDataCorrelation3d_approxiFlatSurf_ex2_HalfHalf(config,data)
...
%             end
            ...
            figure(3);
            [fitParam3_2nd, X1FIT, X2FIT, YFIT , fitR_2nd, yhat, fitR2_2nd ]  = Rhythm.approxiSurface_flat(X1_zc , X2_zc , Y_zc );
            plot3( X1_zc  , X2_zc , Y_zc,   'Marker','*', 'LineStyle','none');
            hold on
                plot3( X1_nzc  , X2_nzc , Y_nzc, 'Marker','o', 'LineStyle','none');
                mesh(X1FIT,X2FIT,YFIT, 'faceAlpha',0.5);
            ...
            title({['V = (' num2str(fitParam3_2nd(1)) ') + (' num2str( fitParam3_2nd(2) ) ') * dPeriod + (' num2str( fitParam3_2nd(3)) ') * dPeak']; ...
                    ['データ数:' num2str( length(user.time.highSampled)  - obj.config.analyzeTime(1) ) '[ms],(' num2str( length(Y_zc) ) ')' ] });
            axis square
            MonitorSize = [ 0, 0, 800, 800];
            set(gcf, 'Position', MonitorSize);
...
%             output = num2cell( [fitParam3' VIF fitR fitR2] ; [fitParam3_2nd' VIF fitR_2nd fitR2_2nd] );
%             
%             if ~isempty(strfind( char(obj.config.examType) , '剣道対戦'))
