classdef ZCDataCorrelation3d_seekAxis_ex_HalfHalf < Analyze.Base
    ...
    methods
        function obj = ZCDataCorrelation3d_seekAxis_ex_HalfHalf(config,data)
...
%             fitParam_recalc = [ beta, alpha*k1, alpha*k2];
            ...
            if ~isempty(strfind( char(obj.config.examType) , '剣道対戦'))
                ...
                obj.saveGraphWithName('_主成分分析_1st-half');
            end
            
%%        3次元分布　重回帰
            figure(100);
%             [fitParam3d, dTFIT, dAFIT, YFIT , fitR, yhat, fitR2 ]  = Rhythm.approxiSurface_flat(dT_zc , dA_zc_shift , Y_zc );
            plot3( dT0_zc  , dA0_zc , Y_zc,   'Marker','*', 'LineStyle','none');
            % 
            hold on
                plot3( dT_nzc - mean(dT_zc) , dA_nzc - mean(dT_zc), Y_nzc, 'Marker','o', 'LineStyle','none');
...
%                 plot3( [0,600*k1]  , [0,600*k2] , [0,0] ,'r' );
                plot3( lineEdgePoint_X1Y(:,1)*k1  , lineEdgePoint_X1Y(:,1)*k2 , lineEdgePoint_X1Y(:,2) ,'r' ,'LineWidth',3 );

                plot3( [ lineEdgePoint_X1Y(2,1)*k1 lineEdgePoint_X1Y(2,1)*k1 ] ,...
...
%%
            ...
            if ~isempty(strfind( char(obj.config.examType) , '剣道対戦'))
                ...
                obj.saveGraphWithName('_重回帰分析（V-X1）_1st-half');
            end
...
%             fitParam_recalc = [ beta, alpha*k1, alpha*k2];
            ...
            if ~isempty(strfind( char(obj.config.examType) , '剣道対戦'))
                ...
                obj.saveGraphWithName('_主成分分析_2nd-half');
            end
...
%%
            ...
            if ~isempty(strfind( char(obj.config.examType) , '剣道対戦'))
                ...
                obj.saveGraphWithName('_重回帰分析（V-X1）_2nd-half');
            end
