classdef ZCDataCorrelation3d_Topview0922 < Analyze.Base
    ...
    methods
        function obj = ZCDataCorrelation3d_Topview0922(config,data)
...
%             end
            ...
            IndexFormer = find(Time(:) > 0 & Time(:) < 30000);
            IndexLater = find(Time(:) > 35000 & Time(:) < 60000);
...
%             hold off
%             
            plot3( dT_zcF  , dA_zcF , Y_zcF,   'Marker','*', 'LineStyle','none','MarkerSize',2);
            hold on
...
%             set(gcf, 'Position', [0 0 500 500]);
           ...
           if ~isempty(strfind( char(obj.config.examType) , '剣道対戦'))
                ...
                obj.saveGraphWithName('_Topview-F');
           end
           clf reset
            %%           主成分回帰分析2
            set(gcf, 'Position', [ 0, 0, 500, 500]);    bFig = 1;
...
%             hold off
            plot3( dT_zcL  , dA_zcL , Y_zcL,   'Marker','*', 'LineStyle','none','MarkerSize',2);
            hold on
%                 plot3(dT_nzc, dA_nzc , Y_nzc ,'Color' , 'b' ,'Marker','o', 'LineStyle','none' );
            hold off
            grid on
...
%             set(gcf, 'Position', [0 0 500 500]);
           ...
           if ~isempty(strfind( char(obj.config.examType) , '剣道対戦'))
                ...
                obj.saveGraphWithName('_Topview-L');
            end
            clf reset
        end
