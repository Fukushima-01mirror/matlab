classdef ZCDataCorrelation3d_seekAxis_ex_small0513 < Analyze.Base
    ...
    methods
        function obj = ZCDataCorrelation3d_seekAxis_ex_small0513(config,data)
        ...
        function runForAlone(obj,user)
           ...
           [zeroCrossTimes] = Rhythm.setZeroCrossCount(user.zeroCrossData);
            ...
            IndexFormer = find(Time(:) <= 30000);
            IndexLater = find(Time(:) > 30000);
            
            Y_zcF  = Y(IndexFormer);        Y_nzcF  = Y(IndexFormer);
            dT_zcF = dT(IndexFormer,:);     dT_nzcF = dT(IndexFormer,:);
            dA_zcF = dA(IndexFormer,:);     dA_nzcF = dA(IndexFormer,:);
            Time_zcF = Time(IndexFormer);   Time_nzcF  = Time(IndexFormer);
            
            Y_zcL  = Y(IndexLater);        Y_nzcL  = Y(IndexLater);
            dT_zcL = dT(IndexLater,:);     dT_nzcL = dT(IndexLater,:);
            dA_zcL = dA(IndexLater,:);     dA_nzcL = dA(IndexLater,:);
            Time_zcL = Time(IndexLater);   Time_nzcL  = Time(IndexLater);
...
%%           主成分回帰分析
            ...
            [k1 , k2, X1, fitParam_X1Y, fitLineR_X1Y] = Rhythm.PCA_regress_3dplot(dT_zcF , dA_zcF , Y_zcF , bFig) ;
            hold on
...
%             set(gcf, 'Position', [0 0 500 500]);
           ...
           if ~isempty(strfind( char(obj.config.examType) , '剣道対戦'))
            ...
            %%           主成分回帰分析2
            set(gcf, 'Position', [ 0, 0, 500, 500]);    bFig = 1;
            [k1 , k2, X1, fitParam_X1Y, fitLineR_X1Y] = Rhythm.PCA_regress_3dplot(dT_zcL , dA_zcL , Y_zcL , bFig) ;
            hold on
