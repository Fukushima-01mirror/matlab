classdef ZCDataCorrelation3d_seekAxis_ex_small_0731kai_second < Analyze.Base
    ...
    methods
        function obj = ZCDataCorrelation3d_seekAxis_ex_small_0731kai_second(config,data)
...
%                 
%             [dT_max,dT_imax] = max(dT_zc);     dT_zc( IndexTout)= [];	 dA_zc( IndexTout)= [];     Y_zc(IndexTout)= [];     Time_zc(IndexTout)= [];
%             [dT_max,dT_imax] = max(dT_zc);     dT_zc(IndexAout)= [];
%             dA_zc(IndexAout)= [];     Y_zc(IndexAout)= [];     Time_zc(IndexAout)= [];
            %今だけ
...
%             end
            ...
            IndexFormer = find(Time_zc(:) > 0 & Time_zc(:) < 44000);
            IndexLater = find(Time_zc(:) > 30000 & Time_zc(:) < 60000);
            
            Y_zcF  = Y_zc(IndexFormer);        
            dT_zcF = dT_zc(IndexFormer,:);     
            dA_zcF = dA_zc(IndexFormer,:);     
            Time_zcF = Time_zc(IndexFormer);   
            
            Y_zcL  = Y_zc(IndexLater);        
            dT_zcL = dT_zc(IndexLater,:);     
            dA_zcL = dA_zc(IndexLater,:);   
            Time_zcL = Time_zc(IndexLater);   
