classdef ZCData_3dPlot_axis_ColorShotsSp < Analyze.Base
    ...
    methods
        function obj = ZCData_3dPlot_axis_ColorShotsSp(config,data)
...
%             spotTimeSeries = spotTimeSeries.';
                    ...
                    startIndexC = find( user.zeroCrossData.zeroCrossTime(IndexZeroCross) >= 45000 ,1 ,'first');
                    endIndexC = find( user.zeroCrossData.zeroCrossTime(IndexZeroCross) < 55000 ,1 ,'last');
                    IndexSP = find(5000 <= user.zeroCrossData.zeroCrossTime(IndexZeroCross) < 23000|...
                        42000 <= user.zeroCrossData.zeroCrossTime(IndexZeroCross) < 60000 )
...
%                     dA0C_zc = dA_zc - dAgC_zc;
                     ...
                     dTgSp_zc = mean(dT_zc(IndexSP));
                    dAgSp_zc = mean(dA_zc(IndexSP));
                    dT0Sp_zc = dT_zc - dTgSp_zc;
                    dA0Sp_zc = dA_zc - dAgSp_zc;
...
%                         plot3( dT_zc(startIndexC:endIndexC), dA_zc(startIndexC:endIndexC), Y_zc(startIndexC:endIndexC),'Color' , 'b' ,'Marker','*', 'LineStyle','none');
                        plot3( dT_zc(IndexSP), dA_zc(IndexSP), Y_zc(IndexSP),'Color' , 'b' ,'Marker','*', 'LineStyle','none');
           ...
           % plot3( [ lineEdgePoint_X1Y(2,1) + mean(dT_zc(startIndexA:endIndexA)) ,lineEdgePoint_X1Y(2,1)+ mean(dT_zc(startIndexA:endIndexA)) ] ,...
               % [ lineEdgePoint_X1Y(2,2)+ mean(dA_zc(startIndexA:endIndexA)) , lineEdgePoint_X1Y(2,2)+ mean(dA_zc(startIndexA:endIndexA)) ] ,...
                %[0 lineEdgePoint_X1Y(2,3)] ,'--r' );              % 垂線
                      [coeffSP,scoreSP,latentSP,tsquareSP] = princomp( [dT_zc(IndexSP) dA_zc(IndexSP)] );
                    k1 = abs(coeffSP(1,1));            k2 = abs(coeffSP(2,1));
                    X1 = k1 * dT0Sp_zc + k2 * dA0Sp_zc;
                    [ fitParam_X1Y, fitLineR_X1Y, lineEdgePoint_X1Y] =Rhythm.approxiLine2d(X1(IndexSP), Y_zc(IndexSP));
            plot3(  [ -100*k1 ; 500*k1 ] + mean(dT_zc(IndexSP)) , [ -100*k2 ; 500*k2 ] + mean(dA_zc(IndexSP)) , zeros(2,1) , 'r' );
              plot3( lineEdgePoint_X1Y(:,1)*k1 +dTgSp_zc , lineEdgePoint_X1Y(:,1)*k2 +dAgSp_zc, lineEdgePoint_X1Y(:,2) ,'r' ,'LineWidth',3 );
               plot3( [ lineEdgePoint_X1Y(2,1)*k1+dTgSp_zc lineEdgePoint_X1Y(2,1)*k1+dTgSp_zc ] ,...
                               [ lineEdgePoint_X1Y(2,1)*k2+dAgSp_zc lineEdgePoint_X1Y(2,1)*k2+dAgSp_zc ] ,...
                               [0 lineEdgePoint_X1Y(2,2)] ,'--r' );              % 垂線
