classdef ZCDataCorrelation3d_approxiStdSurf_ex < Analyze.Base
    ...
    methods
        function obj = ZCDataCorrelation3d_approxiStdSurf_ex(config,data)
        ...
        function runForAlone(obj,user)
           ...
           [zeroCrossTimes] = Rhythm.setZeroCrossCount(user.zeroCrossData);
            ...
            [Y_zc , m_Y , s_Y]  = zscore( Y(IndexZeroCross));
            [X1_zc , m_X1 , s_X1] = zscore( X1(IndexZeroCross,:));
            [X2_zc , m_X2 , s_X2] = zscore( X2(IndexZeroCross,:));

            Y_nzc  =( Y(IndexNonZeroCross) - repmat(m_Y ,length( Y(IndexNonZeroCross)),1 ) )./ s_Y;
            X1_nzc =( X1(IndexNonZeroCross,:) - repmat(m_X1 ,length( X1(IndexNonZeroCross)),1 ) )./ s_X1(3);
            X2_nzc =( X2(IndexNonZeroCross,:)- repmat(m_X2 ,length( X2(IndexNonZeroCross)),1 ) )./ s_X2(3);
            
 
            figure(3);
            [fitParam3, X1FIT, X2FIT, YFIT  , fitR, yhat, fitR2]  = Rhythm.approxiSurface(X1_zc(:,3), X2_zc(:,3), Y_zc );
            ...
            %%      近似面　係数　エクセルデータ出力  
            VIF = Rhythm.getVIF([X1_zc(:,3), X2_zc(:,3), X1_zc(:,3).*X2_zc(:,3)]);
