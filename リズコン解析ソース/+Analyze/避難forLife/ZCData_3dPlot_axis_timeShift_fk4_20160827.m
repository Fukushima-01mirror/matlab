classdef ZCData_3dPlot_axis_timeShift_fk4_20160827 < Analyze.Base
    ...
    methods
        function obj = ZCData_3dPlot_axis_timeShift_fk4_20160827(config,data)
...
%             IndexSoukanPlus_ZC_1P = [];
%             IndexSoukanMinus_ZC_1P = [];
%             IndexSoukanPlus_ZC_2P = [];
%             IndexSoukanMinus_ZC_2P = [];
%             
...
%                       plot([spotTimeMax-spotTime+1 spotTimeMax-spotTime+1],[0 1000],'--k');
  %                      plot([spotTimeMax spotTimeMax],[0 1000],'--k');
                        ...
                        x = user1.time.highSampled;
                        y1 = abs(user2.avatarPosition.highSampled - user1.avatarPosition.highSampled);
                        y2 = user1.avatarPosition.highSampled;
                        y3 = user1.avatarVelocity.highSampled;
                        y4 = user2.avatarPosition.highSampled;
                        y5 = user2.avatarVelocity.highSampled;
                        
                        [hAx,hLine1,hLine2] = plotyy(x,y2,x,y3);
                        set(hLine1,'Color','blue');
                        xlim(hAx(1),[0 60000]);
                        ylim(hAx(1),[0 1000])
                        set(hLine2,'Color','cyan');
                        xlim(hAx(2),[0 60000]);
                        ylim(hAx(2),[-0.5 0.5])
%                         hLine1.LineStyle = '--';
%                         hLine2.LineStyle = ':';
                        hold on
                        [hAx,hLine3,hLine4] = plotyy(x,y4,x,y5);
                        set(hLine3,'Color','green');
                        xlim(hAx(1),[0 60000]);
                        ylim(hAx(1),[0 1000])
                        set(hLine4,'Color','magenta');
                        xlim(hAx(2),[0 60000]);
                        ylim(hAx(2),[-0.5 0.5])
                        hold on 
                       [hAx,hLine3,hLine4] = plotyy(x,y1,x,y5);
...
%                         plot(user1.time.highSampled, ...
%                             abs(user2.avatarPosition.highSampled - user1.avatarPosition.highSampled),':','Color','r','LineWidth',4);
%                         
%                         plotyy(user1.time.highSampled,user1.avatarPosition.highSampled,user1.time.highSampled , user1.avatarVelocity.highSampled);
%                          plotyy(user2.time.highSampled,user2.avatarPosition.highSampled,user2.time.highSampled , user2.avatarVelocity.highSampled);
                        
%                         c = plot( user2.time.highSampled , user2.avatarPosition.highSampled , 'g');
%                         
%                         d = plot( user1.time.highSampled , user1.avatarVelocity.highSampled , 'c');
%                         
%                         e = plot( user2.time.highSampled , user2.avatarVelocity.highSampled , 'm');
...
%                     legend([a,c,d,e,b],'1P', '2P' ,'1P速度','2P速度','距離','Location', 'NorthWest'); 
