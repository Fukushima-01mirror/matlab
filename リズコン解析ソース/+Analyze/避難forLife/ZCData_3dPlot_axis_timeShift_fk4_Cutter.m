classdef ZCData_3dPlot_axis_timeShift_fk4_Cutter< Analyze.Base
    ...
    methods
        function obj = ZCData_3dPlot_axis_timeShift_fk4_Cutter(config,data)
...
%                       plot([spotTimeMax-spotTime+1 spotTimeMax-spotTime+1],[0 1000],'--k');
  %                      plot([spotTimeMax spotTimeMax],[0 1000],'--k');
                    ...
                    q1 = round(user1.time.highSampled(1)/5000);
                    q2 = ceil(user1.time.highSampled(end)/5000);
                    S_time = q1*5000;
                    E_time = (q2+1)*5000;
                    xlim([S_time E_time]);    ylim([0 1000]);
