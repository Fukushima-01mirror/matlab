classdef ZCData_3daxis_timeShift_1F < Analyze.Base
    ...
    methods
        function obj = ZCData_3daxis_timeShift_1F(config,data)
...
%                 [vector_HP_g2_2 , distError_g2_2 , distMean_g2_2] = Rhythm.PCA_regDist(dT_zc2 , dA_zc2 , Y_zc2 , k1_g2_2, k2_g2_2 , X1_g2_2, mean([dT_zc2(indexG2_2) , dA_zc2(indexG2_2)]) ,  fitParam_X1Y_g2_2, bFig );                
            ...
            %クラスタ
                [k1_2 , k2_2, X1_2, fitParam_X1Y_2, fitLineR_X1Y_2] = Rhythm.PCA_regress_3dplot(dT_zc2 , dA_zc2 , Y_zc2 , bFig);
                
                index_spot_2 = startIndex2:endIndex2;
                index_nspot_2 = [1:startIndex2-1, endIndex2+1:length(IndexZeroCross2)];
                
                lineEdgePoint_X1Y_2 = [ min(X1_2)*k1_2 , min(X1_2)*k2_2 , polyval( fitParam_X1Y_2, min(X1_2) ) ;...
                                        max(X1_2)*k1_2 , max(X1_2)*k2_2 , polyval( fitParam_X1Y_2, max(X1_2) ) ];
                lineEdgePoint_X1Y_2 = [ min(X1_2)*k1_2 , min(X1_2)*k2_2 , polyval( fitParam_X1Y_2, min(X1_2) ) ;...
                                        max(X1_2)*k1_2 , max(X1_2)*k2_2 , polyval( fitParam_X1Y_2, max(X1_2) ) ];
                plot3( [0 0],[0 0],[0 0],'k' );
                hold on
                    ...
                    plot3(dT_zc2(index_spot_2) , dA_zc2(index_spot_2) , Y_zc2(index_spot_2) ,'MarkerEdgeColor' , 'b' ,'MarkerFaceColor','b', 'Marker','o', 'LineStyle','none','MarkerSize',5);
%                     plot3(dT_zc2(indexGd2_spot_2) , dA_zc2(indexGd2_spot_2) , Y_zc2(indexGd2_spot_2) ,'MarkerEdgeColor' , [0 .5 0] ,'MarkerFaceColor',[0 .5 0], 'Marker','o', 'LineStyle','none','MarkerSize',5);
                    ...
                    plot3(  [ -100*k1_2 ; 500*k1_2 ] + mean(dT_zc2) , [ -100*k2_2 ; 500*k2_2 ] + mean(dA_zc2) , zeros(2,1) , 'r');
                    plot3( lineEdgePoint_X1Y_2(:,1) + mean(dT_zc2) , lineEdgePoint_X1Y_2(:,2) + mean(dA_zc2), lineEdgePoint_X1Y_2(:,3) ,'r' ,'LineWidth',3 );
                    plot3( [ lineEdgePoint_X1Y_2(2,1) + mean(dT_zc2) ,lineEdgePoint_X1Y_2(2,1)+ mean(dT_zc2) ] ,...
                           [ lineEdgePoint_X1Y_2(2,2)+ mean(dA_zc2) , lineEdgePoint_X1Y_2(2,2)+ mean(dA_zc2) ] ,...
                           [0 lineEdgePoint_X1Y_2(2,3)] ,'--r' );              % 垂線
                ...
                x_2 = [ -100*k1_2 ; 500*k1_2 ] + mean(dT_zc2);
                y_2 = [ -100*k2_2 ; 500*k2_2 ] + mean(dA_zc2);
                dx_2 = x_2(2,1)-x_2(1,1);
                dy_2 = y_2(2,1)-y_2(1,1);
                x2_2 = [ -100*k1_2 ; 500*k1_2 ] + mean(dT_zc2);
                y2_2 = [ -100*k2_2 ; 500*k2_2 ] + mean(dA_zc2);
...
%                         disp(strcat(num2str(time_s(n_group)),',',num2str(time_e(n_group)),',',num2str(n_group)));
                    ...
                    hold on
                        ...
                        a = plot(   user1.time.highSampled,  user1.avatarPosition.highSampled , 'color', 'b');
            ...
            %% 速度分布
                    ...
                    xlim([0,maxYaxis]);        ylim([0,0.4]);

                    %2P
                    subplot(4,10,[31:35]);
                    
                    nY_zc2_spot = hist( Y_zc2(index_spot_2) ,Ycenters );
                    nY_zc2_nspot = hist( Y_zc2(index_nspot_2) ,Ycenters );
                    
                    N2 = sum(nY_zc2_spot) + sum(nY_zc2_nspot);
                    hBarY2 = bar(Ycenters.' ,  [nY_zc2_spot.'/N2 nY_zc2_nspot.'/N2], 'Stack');
                    set(hBarY2(2),'FaceColor', 'w' );
                    ylabel('度数');   xlabel('対数演算前アバタ速さ');
                    xlim([0,maxYaxis]);        ylim([0,0.4]);
...
%                     end
    ...
    %                 plot([spotTimeMax spotTimeMax],[0 1000],'--k');
                ...
                xlabel('時間t ms'); ylabel('アバタ位置');
%                 xlim([19000 26500]);    ylim([0 1000]);
                xlim([(j-1)*60000/n_separate j*60000/n_separate]);    ylim([0 1000]);
            end
            
            obj.saveGraphWithNameToFolder( ['avatar_position']);
...
%             if ~isempty(  findstr( char( obj.config.examType ) , '剣道'))
                AB = AB_Series_Max(:,2);
                AB = AB(~isnan(AB));
                title( [ 'コントローラ操作波形　最大相互相関  mean=' num2str(mean(AB))]);
%             end
            axes(haxes(1))
            xlabel('時間t ms'); ylabel('コントローラ操作');
            xlim([startt endt]);    ylim([-400 400]);
            set(gca,'YTick',[-400:200:400]);

            axes(haxes(2))
            legend([hline1,a,hline2],'1P','2P', '最大相互相関','Location', 'NorthWest');
            xlabel('時間t ms'); ylabel('最大相互相関');
            xlim([startt endt]);    ylim([0 1]);
            set(gca,'YTick',[0:0.2:1]);
                
            obj.saveGraphWithNameToFolder( [ num2str( obj.data.splitTimeInfo.index ) '_' obj.data.splitTimeInfo.state '_picked_area'  ]);
