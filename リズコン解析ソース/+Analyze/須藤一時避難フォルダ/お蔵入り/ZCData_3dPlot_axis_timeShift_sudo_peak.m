classdef ZCData_3dPlot_axis_timeShift_sudo_peak < Analyze.Base
    ...
    methods
        function obj = ZCData_3dPlot_axis_timeShift_sudo_peak(config,data)
...
%                 xlim([spotTimeMax-3500-2500+1250 spotTimeMax+3500-2500+1250]);    ylim([0 600]);
                ...
                a = plot( user1.zeroCrossData.zeroCrossTime , abs(peak_zx(:,3)) , 'b');
                    
                hold on 
                    plot([spotTimeMax-spotTime+1 spotTimeMax-spotTime+1],[0 1000],'--k');
                    plot([spotTimeMax spotTimeMax],[0 1000],'--k');
                    b = plot( user2.zeroCrossData.zeroCrossTime , abs(peak_zx2(:,3)) , 'g');

                hold off

                title( [ 'コントローラ操作　振幅差  ( ' char( obj.data.splitTimeInfo.state ) ' ）']);
                legend([a,b],'1P', '2P','Location', 'NorthWest');
                xlabel('時間t ms'); ylabel('振幅差ΔA ms');
                xlim([spotTimeMax-3500-2500+1250 spotTimeMax+3500-2500+1250]);    ylim([0 600]);
