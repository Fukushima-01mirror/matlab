function [ obj ] = Loader_Kendo_ofw( config )
...
%            obj.device = Models.Device(deviceFileName);
    ...
    if config.isExistPlayer1() && config.isExistPlayer2()      %二人で実験 剣道
        ...
        match_list2 =  dir([dataDirName,'match2_*.csv']);
        ...
        obj.player1 = Models.RhythmAvatar_splitTime_ofw(dataDirName , match_data_1 ,'1');
        if ~isempty( match_list2 )
            obj.player2 = Models.RhythmAvatar_splitTime_ofw(dataDirName , match_data_2 ,'2');
        else
            obj.player2 = Models.RhythmAvatar_splitTime_ofw(dataDirName , match_data_1 ,'2');
        end
    else
