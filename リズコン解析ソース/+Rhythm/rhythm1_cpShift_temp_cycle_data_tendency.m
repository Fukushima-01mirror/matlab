function[avt_temp_data, cycle_temp_data] = rhythm1_cpShift_temp_cycle_data_tendency(x0, Pul, crossData, cnst_a, cnst_b, Pul0, bCalc)
...
devPul = Pul + Pul0 * ones(n_data,1) - crossData;   
...
for t= 2 : n_data
    ...
    % ゼロクロス時
        ...
        cycle_temp_data(j,2) = S_Pul(j,2);    %コントローラ操作積算値
        
        if j>1 && S_Pul(j-1,2) ~= 0   % アバター速度計算
            v_avt(t) = S_Pul(j,2) + S_Pul(j-1,2);
            cycle_temp_data(j,3) = v_avt(t);   %対数演算前アバタ速度
            if v_avt(t)>0
            % アバター位置計算
                avt_temp_data(t,1) = - 0.75 * cnst_a *log(v_avt(t)/cnst_b +1)*0.001;
            else
                avt_temp_data(t,1) = + 1.25 * cnst_a *log(-v_avt(t)/cnst_b +1)*0.001;
            end
            cycle_temp_data(j,4) = avt_temp_data(t,1);  %アバタ速度
        end
