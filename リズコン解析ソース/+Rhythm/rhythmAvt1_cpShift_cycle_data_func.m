...
tempPul = tempPul - round( mean(tempPul(1:60000)) );

for Pul0=1:0.5:100     %コントローラ操作量の初期値
%     disp('Pul0=');disp(Pul0);
        ...
        if signPul0 == 1      %初期値マイナス 
            ...
            if max(devPul) > 0
                [avt_temp_data1, cycle_data1] ...
                    = Rhythm.rhythm1_cpShift_temp_cycle_data(data0(1,2), tempPul, crossData, cnst_a, cnst_b, -(Pul0-1), 0);
                [avt_temp_data2, cycle_data2] ...
                    = Rhythm.rhythm1_cpShift_temp_cycle_data(data0(1,2), tempPul, crossData, cnst_a, cnst_b, -(Pul0-1), 1);
            else
                bMinusNonCross =1;  
                disp('Apoint');
                break;
...
%         end
        ...
        if signPul0 == 1 && Pul0 > 1    %初期値マイナス
            if ( bMinus || bPlusNonCross ) && ( Err(k-2,1) < 30 || Err(k-2,2) <35 ) %前回誤差小
                %20,25
        ...
        elseif signPul0 == 2 && Pul0 > 1    %初期値プラス
            if ( bPlus || bMinusNonCross )&& ( Err(k-2,1) < 30 || Err(k-2,2) <35 ) %前回誤差小
                bErrIncrease = Err(k,1)- Err(k-2,1) >0; %誤差が大きくなったら，bErrIncreaseがtrueに
                bPlusC = true;
            else
        ...
        k=k+1;
    end     % for signPul0 =1:2　閉じ( Aポイント,Bポイント,Cポイントのbreak先)
    
    if Pul0 > 1 && bMinus==0 && bPlus==0&& Err(1,1) < 35
...
if Pul0 ==2
    [avt_temp_data1, cycle_data1] ...
     = Rhythm.rhythm1_cpShift_temp_cycle_data(data0(1,2), tempPul, crossData, cnst_a, cnst_b, (Pul0-2), 0);
    [avt_temp_data2, cycle_data2] ...
     = Rhythm.rhythm1_cpShift_temp_cycle_data(data0(1,2), tempPul, crossData, cnst_a, cnst_b, (Pul0-2), 1);
