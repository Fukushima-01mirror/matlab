function varargout = simplots(time, data, ports, diffStr)
...
for i=1:nStructs
  ...
  elseif exist('noTimeVector')==1 %#ok<*EXIST>
...
for i=1:nAxes,
  hAxes(i,1) = subplot(nAxes, 1, i); %#ok<AGROW>
...
if isempty(data.time)
  xString = getString(message('Simulink:blocks:ScopeIndices'));
  data.time = (1:size(data.signals(1).values, 1))';
else
  xString = getString(message('Simulink:blocks:ScopeTime'));
end
