...
%% settings
numGroup = 4;
analysisName = 'sameMotion_differCondition';
...
%% データグループ分け
    ...
    for i = 1: length(excel.num)
        ...
        %dataGroup1ソート条件
        if isempty(strfind( char( excel.txt(i+1,1) ) , 'ランダム')) ...         %ランダムじゃない
                && excel.num(i,3) == 0 && excel.num(i,5)< 0.05 ...          %動き：sin　誤差が小　
                && excel.num(i,4) ==1                                       %前進
            [dataGroup1] = Rhythm.storeDataGroup(dataGroup1, data.player1, excel.num(i,1), excel.num(i,2), j1);
            j1=j1+1;
        end
        %dataGroup2ソート条件
        if isempty(strfind( char( excel.txt(i+1,1) ) , 'ランダム')) ...         %ランダムじゃない
                && excel.num(i,3) == 0 && excel.num(i,5)< 0.05...               %動き：sin　誤差が小　
                && excel.num(i,4) ==-1                                          %後退
            [dataGroup2] = Rhythm.storeDataGroup(dataGroup2, data.player1, excel.num(i,1), excel.num(i,2), j2);
            j2=j2+1;
        end
        %dataGroup3ソート条件
        if ~isempty(strfind( char( excel.txt(i+1,1) ) , 'ランダム')) ...         %ランダム
                && excel.num(i,3) == 0 && excel.num(i,5)< 0.05 ...          %動き：sin　誤差が小　
                && excel.num(i,4) ==1                                       %前進
            [dataGroup3] = Rhythm.storeDataGroup(dataGroup3, data.player1, excel.num(i,1), excel.num(i,2), j3);
            j3=j3+1;
        end
        %dataGroup4ソート条件
        if ~isempty(strfind( char( excel.txt(i+1,1) ) , 'ランダム')) ...         %ランダム
                && excel.num(i,3) == 0 && excel.num(i,5)< 0.05 ...          %動き：sin　誤差が小　
                && excel.num(i,4) ==-1                                          %後退
            [dataGroup4] = Rhythm.storeDataGroup(dataGroup4, data.player1, excel.num(i,1), excel.num(i,2), j4);
            j4=j4+1;
        end    
...
for i= 1:numGroup
    ...
    if i==1
        Models.saveGraphWithName(saveDir, 'アバタ操作分布_常にsin(前進)')
    elseif i==2
        Models.saveGraphWithName(saveDir, 'アバタ操作分布_常にsin(後退)')
    elseif i==3
        Models.saveGraphWithName(saveDir, 'アバタ操作分布_ランダムsin(前進)')
    elseif i==4
        Models.saveGraphWithName(saveDir, 'アバタ操作分布_ランダムsin(後退)')
    end
