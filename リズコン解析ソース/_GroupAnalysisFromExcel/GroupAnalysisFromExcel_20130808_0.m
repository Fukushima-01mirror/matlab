...
%% settings
numGroup = 2;
analysisName = 'sameMotion_differCondition0802';
...
if exist([saveDir '\dataGroup.mat'],'file') && alwaysLoadRawData ~=1
    ...
    for i = 1: length(excel.num)
        ...
        if isempty(strfind( char( excel.txt(i+1,1) ) , 'ランダム')) ...         %ランダムじゃない
                && excel.num(i,3) == 0 && excel.num(i,2)-excel.num(i,1)==2500 && excel.num(i,5)< 0.2                  %動き：sin　誤差が小　
        ...
        if ~isempty(strfind( char( excel.txt(i+1,1) ) , 'ランダム')) ...         %ランダム
                && excel.num(i,3) == -0.006 && excel.num(i,2)-excel.num(i,1)==2500 && excel.num(i,5)< 0.2                  %動き：sin　誤差が小　
...
for i= 1:numGroup
    ...
    if i==1
        Models.saveGraphWithName(saveDir, 'アバタ操作分布_常にsin')
    elseif i==2
        Models.saveGraphWithName(saveDir, 'アバタ操作分布_ランダムsin')
    end
