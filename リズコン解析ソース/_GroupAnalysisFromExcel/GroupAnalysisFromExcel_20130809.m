...
if exist([saveDir '\dataGroup.mat'],'file') && alwaysLoadRawData ~=1
    ...
    for i = 1: length(excel.num)
        ...
        %dataGroup1ソート条件
        if excel.num(i,3) == 0.16 && excel.num(i,2)-excel.num(i,1)==2500 && excel.num(i,5)< 0.075 ...          %動き：sin　誤差が小　
                && excel.num(i,4) ==1                                       %前進
            suffix = 1;
...
for i= 1:numGroup
    ...
    if i==1
        Models.saveGraphWithName(saveDir, 'アバタ操作分布_ランダム低速(前進)')
    elseif i==2
        Models.saveGraphWithName(saveDir, 'アバタ操作分布_ランダム低速(後退)')
    end
