%条件別(定常orランダム,前後）データの描画 相関分布図のみ
%% Excelデータからソートして解析
clf;
% clearvars *;
clc;
...
if exist([saveDir '\dataGroup.mat'],'file') && alwaysLoadRawData ~=1
    ...
    for i = 1: length(excel.num)
        ...
        %dataGroupソート条件
        if isempty(strfind( char( excel.txt(i+1,1) ) , 'ランダム')) ...         %速度定常
                && excel.num(i,5)< 0.06 ...          %誤差が小　
                && excel.num(i,4) ==1                                       %前進
            suffix = 1;
            eval(['[dataGroup' num2str(suffix) '] = Rhythm.storeDataGroup_zeroCross(dataGroup'...
        ...
        %dataGroupソート条件
        if ~isempty(strfind( char( excel.txt(i+1,1) ) , 'ランダム')) ...         %速度ランダム
                && excel.num(i,5)< 0.06 ...              %誤差が小　
                && excel.num(i,4) ==1                                       %前進
            suffix = 3;
            eval(['[dataGroup' num2str(suffix) '] = Rhythm.storeDataGroup_zeroCross(dataGroup'...
...
for i= 1:numGroup
    ...
    if i==1
        Models.saveGraphWithName(saveDir, 'アバタ操作分布_sin速度(前進)')
    elseif i==2
        Models.saveGraphWithName(saveDir, 'アバタ操作分布_sin速度(後退)')
    elseif i==3
        Models.saveGraphWithName(saveDir, 'アバタ操作分布_ランダム速度(前進)')
    elseif i==4
        Models.saveGraphWithName(saveDir, 'アバタ操作分布_ランダム速度(後退)')
    end
