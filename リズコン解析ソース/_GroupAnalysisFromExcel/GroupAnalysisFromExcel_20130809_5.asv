...
if exist([saveDir '\dataGroup.mat'],'file') && alwaysLoadRawData ~=1
    ...
    for i = 1: length(excel.num)
        ...
        %dataGroup1ソート条件
        if ~isempty(strfind( char( excel.txt(i+1,1) ) , '速め')) ...         %速め
        ...
        %dataGroup2ソート条件
        if ~isempty(strfind( char( excel.txt(i+1,1) ) , '普通')) ...         %普通
        ...
        %dataGroup3ソート条件
        if ~isempty(strfind( char( excel.txt(i+1,1) ) , '遅め')) ...         %遅め
...
for i= 1:numGroup
    ...
    if i==1
        Models.saveGraphWithName(saveDir, 'アバタ操作分布_sin往復(速め)')
    elseif i==2
        Models.saveGraphWithName(saveDir, 'アバタ操作分布_sin往復(普通)')
    elseif i==3
        Models.saveGraphWithName(saveDir, 'アバタ操作分布_sin往復(遅め)')
    end
