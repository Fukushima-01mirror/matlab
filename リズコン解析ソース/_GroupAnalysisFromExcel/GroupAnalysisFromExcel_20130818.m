...
%% settings
numGroup = 3;
groupName1 = 'ランダム速度(sin)';
groupName2 = 'ランダム速度(遅め0.16)';
groupName3 = 'ランダム速度(速め0.20)';
...
if exist([saveDir '\dataGroup.mat'],'file') && alwaysLoadRawData ~=1
    ...
    for i = 1: length(excel.num)
        ...
        %dataGroupソート条件
        if ~isempty(strfind( char( excel.txt(i+1,1) ) , 'ランダム')) ...         %ランダム　速度sin
        ...
        %dataGroupソート条件
        if ~isempty(strfind( char( excel.txt(i+1,1) ) , 'ランダム')) ...         %ランダム　速度0.16
        ...
        %dataGroupソート条件
        if ~isempty(strfind( char( excel.txt(i+1,1) ) , 'ランダム')) ...         %ランダム　速度0.16
                && excel.num(i,2)-excel.num(i,1)==2000  && excel.num(i,3) ==0.20 ...
                && excel.num(i,5)< 0.075           %誤差が小　
            suffix = 3;
            eval(['[dataGroup' num2str(suffix) '] = Rhythm.storeDataGroup(dataGroup'...
                num2str(suffix) ', data.player1, excel.num(i,1), excel.num(i,2), j' num2str(suffix) ');']);
