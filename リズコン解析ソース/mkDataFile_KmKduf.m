...
UserNamePlayer2 = '';
contParam2 = [NaN NaN NaN NaN NaN];
bContModeList2 = 0;
ExamTime = '60000';
bExamTypeList = 0;
...
for n_list = 1: length(avt_list)
    ...
    output(RowNum + n_list,3:8) = num2cell(contParam1); 

    output(RowNum + n_list,9) = cellstr(UserNamePlayer2); 
    output(RowNum + n_list,10:15) = num2cell(contParam2); 
    
    if bExamTypeList ==1
        ExamType = examTypeList(n_list) ; 
    end
    output(RowNum + n_list,16) = cellstr(ExamType); 
    output(RowNum + n_list,17) = cellstr(ExamTime); 
    output(RowNum + n_list,18) = cellstr(acvFolderName); 
